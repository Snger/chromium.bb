...
class GFX_EXPORT AxisTransform2d {
 public:
  ...
  AxisTransform2d(const SizeF& scale)
      : scale_(scale) {}
  AxisTransform2d(const SizeF& scale, const Vector2dF& translation)
  ...
  void PreScale(const SizeF& scale) { scale_.Scale(scale.width(), scale.height()); }
  ...
  PointF InverseMapPoint(const PointF& p) const {
    return ScalePoint(p - translation_, 1.f / scale_.width(), 1.f / scale_.height());
  }
  ...
  RectF InverseMapRect(const RectF& r) const {
    DCHECK(scale_.width() > 0.f && scale_.height() > 0.f);
    return ScaleRect(r - translation_, 1.f / scale_.width(), 1.f / scale_.height());
  }
